...
const RecipeTemplate = (props) => {
  ...
  return (
    <div className='flex flex-col items-center gap-2 '>
      <Link className="w-full lg:w-fit duration-200 hover:scale-90 hover:bg-orange-100 font-[montserrat] p-3 rounded-xl flex flex-col text-center items-center gap-2 h-fit outline">
        <img className='w-48 h-48 rounded-full border object-cover hover:outline' src={image} alt="error404" />
        ...
        <h3 className=' font-bold text-4xl'> {title.toUpperCase()}</h3>
        <p className=' font-[poppins] text-3xl mt-2'>{chefname}</p>
        <p className=' font-serif font-semibold text-2xl'> {catagory}</p>
